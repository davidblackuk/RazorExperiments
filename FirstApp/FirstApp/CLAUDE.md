...
FirstApp is an ASP.NET Core Razor Pages application (.NET 10) for designing and browsing a schema of "repositories" containing "object types," which in turn contain "property types." It uses ASP.NET Core Identity for auth and EF Core with SQLite for persistence. There is no separate solution-wide test project.

The repo root for the app is `FirstApp/FirstApp` (the `.slnx` solution file lives one level up in `FirstApp/`).

## Commands

Run all commands from `FirstApp/FirstApp/` (the project directory containing `FirstApp.csproj`).

- Restore/build: `dotnet build`
- Run the app: `dotnet run` (serves on `http://localhost:5114` and `https://localhost:7184`, per `Properties/launchSettings.json`)
- Apply/update the database: `dotnet ef database update` (requires `dotnet-ef` tool; uses the `DefaultConnection` string in `appsettings.json`, a SQLite file `FirstApp.db` created in the project directory)
